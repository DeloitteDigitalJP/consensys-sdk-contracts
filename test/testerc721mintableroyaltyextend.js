const ERC721MintableRoyaltyExtend = artifacts.require("ERC721MintableRoyaltyExtend");
const {
...
contract("ERC721MintableRoyaltyExtend", async (accounts) => {
    ...
    beforeEach(async () => {
        instance = await ERC721MintableRoyaltyExtend.deployed();
    });
    ...
    it("should have the deployed name and symbol", async () => {
        ...
        assert.equal("My Test Royalty NFT", name);
        assert.equal("MTRNFT", symbol);
    });
    ...
    it("should have default royalties initially", async () => {
    ...
    it("should set token royalties", async () => {
        await instance.setTokenRoyalty(royaltyTokenId1, accounts[1], 100, { from: accounts[0] });
        await instance.setTokenRoyalty(royaltyTokenId2, accounts[2], 200, { from: accounts[0] });
        ...
        const response2 = await instance.royaltyInfo(royaltyTokenId2, 10000);
        assert.equal(accounts[2], response2[0]);
        assert.equal(200, response2[1].toNumber());
    });

    it("should revert as setting receiver to zero address", async () => {
        await expectRevert(
            instance.setTokenRoyalty(royaltyTokenId1, constants.ZERO_ADDRESS, 100),
    ...
    it("should revert as royalties are too high", async () => {
        await expectRevert(
            instance.setTokenRoyalty(royaltyTokenId1, accounts[1], 10001, { from: accounts[0] }),
            "ERC2981: royalty fee will exceed salePrice"
        )
    });
    ...
    it("should revert as caller is not admin", async () => {
        try {
            await instance.setTokenRoyalty(royaltyTokenId1, accounts[1], 100, { from: accounts[1] });
        } catch (e) {
            assert.include(e.message, "is missing role");
        }
    });

    it("should revert as tokenId is not exists", async () => {
        await expectRevert(
